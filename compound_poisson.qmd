...
```{r}
aa <- dd %>% 
  filter(date_time > lubridate::mdy("12/31/1979") & date_time < lubridate::mdy("1/1/2000"))
mm <- min(as.numeric(aa$date_time))
MM <- max(as.numeric(aa$date_time))
ss <- hist(as.numeric(aa$date_time), breaks = seq(mm, MM, length.out = 20))
```
...
```{r}
hist(rnbinom(335, size = sizehat, mu = muhat))
```
